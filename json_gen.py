...
def generate_artist_json(MUSIC_DIR, CAL_DIR):
    ...
    with open(
        os.path.join(MUSIC_DIR, CAL_DIR, "meta", "artists.json"), "w"
    ) as file:  # Save in the "music_index" folder
...
def generate_album_json(artist, MUSIC_DIR, CAL_DIR):
    ...
    with open(
        os.path.join(MUSIC_DIR, CAL_DIR, "albums", f"{artist}_albums.json"), "w"
    ) as file:  # Save in the "music_index" folder
...
def generate_songs_json(artist, album, MUSIC_DIR, CAL_DIR, number_of_songs):
    ...
    for song in songs:
        number_of_songs += 1
        if (
            song.startswith("desktop")
            and song.endswith(".ini")
            and config.UserOptions.IGNORE_DESKTOP_INI
        ):
            pass
        elif (
            song.startswith("Thumbs")
            and song.endswith(".db")
            and config.UserOptions.IGNORE_THUMBS_DB
        ):
            pass
        else:
            ...
            if name.endswith(" UTC)") and config.UserOptions.AUTO_REMOVE_UTC_TIMESTAMP:
                ...
                utc_removed += 1
            song_data.append(
                {"name": name, "path": song, "uri": f"$SERVER/{artist}/{album}/{song}"}
            )

    with open(
        os.path.join(MUSIC_DIR, CAL_DIR, "songs", f"{artist}_{album}_songs.json"), "w"
    ) as file:  # Save in the "music_index" folder
