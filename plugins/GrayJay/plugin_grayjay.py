...
def get_phone_ip():
    result = subprocess.run(
        ["adb", "shell", "ip -4 -o addr show wlan0 | awk '{print $4}'"],
...
def connect_wifi(ip_address):
    ...
    subprocess.run(
        "scrcpy -e -w -t --window-title='GrayJay - Sonic Screwdriver'", shell=True
    )
...
if __name__ == "__main__":
    ...
    if connect_wifi_yes_no == "N":
        subprocess.run(
            "scrcpy -d -w -t --window-title='GrayJay - Sonic Screwdriver'", shell=True
        )
    else:
